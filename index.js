...
const AUTO_RESET_MS  = 3 * 60 * 1000; // reset if timer drops below 3 min
const BOT_NAMES      = ['Duck #3344','Duck #7712','Duck #2291','Duck #5503','Duck #8819','Duck #1147','Duck #6632','Duck #4478','Duck #9021','Duck #3307'];
...
// ═══════════════════ AUTO TIMER RESET ═══════════════════
// Runs every 30 seconds — if timer drops below 3 min
// with no real human click, a "bot" resets it silently.
// This keeps the game alive when player count is low.
async function autoResetCheck() {
  try {
    const state = await getOrInitState();
    const now   = Date.now();
    const remainingMs = Math.max(0, new Date(state.timer_ends_at) - now);

    if (remainingMs > AUTO_RESET_MS) return; // timer still above 3 min, do nothing
    if (now >= EVENT_END.getTime()) return;   // event ended

    // Reset timer silently with a bot name
    const botName    = rBotName();
    const newTimerEnd= new Date(now + TIMER_MS);

    await pool.query(
      `UPDATE game_state SET
         timer_ends_at     = $1,
         last_clicker_name = COALESCE(last_clicker_name, $2),
         total_clicks      = total_clicks + 1,
         updated_at        = $3
       WHERE id = 1`,
      [newTimerEnd, botName, new Date(now)]
    );

    console.log(`Auto-reset: timer was at ${Math.floor(remainingMs/1000)}s, reset to 5min`);
  } catch(e) {
    console.error('Auto-reset check failed:', e.message);
  }
}
...
// Auto-reset check every 30 seconds
const autoResetInterval = setInterval(autoResetCheck, 30000);
