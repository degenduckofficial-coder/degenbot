...
await pool.query(`CREATE TABLE IF NOT EXISTS players ( telegram_id       BIGINT PRIMARY KEY, username          TEXT, first_name        TEXT, tries_remaining   INTEGER NOT NULL DEFAULT 4, tries_earned      INTEGER NOT NULL DEFAULT 0, tries_purchased   INTEGER NOT NULL DEFAULT 0, total_clicks      INTEGER NOT NULL DEFAULT 0, best_click_ms     BIGINT, last_click_at     TIMESTAMPTZ, last_shared_at    TIMESTAMPTZ, joined_group      INTEGER NOT NULL DEFAULT 0, referral_code     TEXT UNIQUE, referred_by       BIGINT, referral_count    INTEGER NOT NULL DEFAULT 0, streak_days       INTEGER NOT NULL DEFAULT 0, last_streak_date  DATE, squad_id          BIGINT, verified_human    INTEGER NOT NULL DEFAULT 0, captcha_fails     INTEGER NOT NULL DEFAULT 0, captcha_locked_until TIMESTAMPTZ, created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW() ); CREATE TABLE IF NOT EXISTS clicks ( id           BIGSERIAL PRIMARY KEY, telegram_id  BIGINT NOT NULL, timer_ms     BIGINT, clicked_at   TIMESTAMPTZ NOT NULL DEFAULT NOW() ); CREATE TABLE IF NOT EXISTS game_state ( id                  INTEGER PRIMARY KEY DEFAULT 1, timer_ends_at       TIMESTAMPTZ NOT NULL, last_clicker_id     BIGINT, last_clicker_name   TEXT, second_clicker_id   BIGINT, second_clicker_name TEXT, third_clicker_id    BIGINT, third_clicker_name  TEXT, total_clicks        INTEGER NOT NULL DEFAULT 0, updated_at          TIMESTAMPTZ NOT NULL DEFAULT NOW() ); CREATE TABLE IF NOT EXISTS invites ( id           BIGSERIAL PRIMARY KEY, inviter_id   BIGINT NOT NULL, invitee_id   BIGINT NOT NULL UNIQUE, credited     INTEGER NOT NULL DEFAULT 0, created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW() ); CREATE TABLE IF NOT EXISTS squads ( id           BIGSERIAL PRIMARY KEY, name         TEXT, creator_id   BIGINT NOT NULL, invite_code  TEXT UNIQUE, created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW() ); CREATE TABLE IF NOT EXISTS purchases ( id           BIGSERIAL PRIMARY KEY, telegram_id  BIGINT NOT NULL, package_id   TEXT NOT NULL, tries_added  INTEGER NOT NULL, stars_paid   INTEGER NOT NULL, created_at   TIMESTAMPTZ NOT NULL DEFAULT NOW() ); CREATE TABLE IF NOT EXISTS captcha_sessions ( telegram_id   BIGINT PRIMARY KEY, correct_emoji TEXT NOT NULL, expires_at    TIMESTAMPTZ NOT NULL, attempts      INTEGER NOT NULL DEFAULT 0 );`);
...
await pool.query(
`INSERT INTO captcha_sessions(telegram_id,correct_emoji,expires_at,attempts) VALUES($1,$2,$3,0) ON CONFLICT(telegram_id) DO UPDATE SET correct_emoji=$2,expires_at=$3,attempts=0`,
...
This is a one-time check — you’ll never see it again after this.`, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [ options.map(e => ({ text: e, callback_data: `captcha_${telegramId}_${e}` }))
]
}
}
);
...
You’re temporarily locked for 10 minutes. Real players only, anon. 🦆`,{ parse_mode: 'Markdown' } ); } else { await bot.sendMessage(telegramId,`❌ Wrong! Try again — ${3-fails} attempt${3-fails!==1?‘s’:’’} remaining.`
);
}
return false;
}
...
`😤 *So close!* You missed the Top 10 by only *${diffSec}s!* #10: *${fmtMs(rows[9].best_click_ms)}* | Yours: *${fmtMs(clickMs)}* Try again — you've got this! 🦆`,
{parse_mode:‘Markdown’}
);
} catch(e){}
}
...
await bot.sendMessage(tid,
`🦆 *$DEGEN 100 SOL BUTTON — MY CARD* ━━━━━━━━━━━━━━━━━━ 👤 ${maskName(p.username,p.first_name)} ⏱ Best Click: *${fmtMs(p.best_click_ms)}* 📊 Rank: ${rankStr} 🖱 Total Clicks: ${p.total_clicks} 🔥 Streak: ${p.streak_days}d ${streakEmoji(p.streak_days)} 👥 Invites: ${p.referral_count} ━━━━━━━━━━━━━━━━━━ 💰 Prize: 100 SOL • Ends Oct 10 2026 *Can you beat me?* 🦆`,
...
*Patience wins. WAGMI 🚀*`,
{parse_mode:‘Markdown’,…kb(p.referral_code)}
);
});
...
const chosen   = parts.slice(2).join(’*’);
...
Choose a package:`, {parse_mode:'Markdown',reply_markup:{inline_keyboard: STAR_PACKAGES.map(p=>[{ text:`${p.emoji} ${p.label} — ${p.stars} ⭐`, callback_data:`buy_pack_${p.id}`
}])
}}
);
return;
...
const packId  = parts[1]+’*’+parts[2]; // e.g. pack_5
const pack    = STAR_PACKAGES.find(p=>p.id===packId);
if (!pack) return;
...
bot.sendMessage(msg.from.id,‘🦆 Tap below!’,kb(p.referral_code));
});
...
Current tries: *${p.tries_remaining}*`, {parse_mode:'Markdown',reply_markup:{inline_keyboard: STAR_PACKAGES.map(p=>[{ text:`${p.emoji} ${p.label} — ${p.stars} ⭐`, callback_data:`buy_pack_${p.id}`
}])
}}
);
});
...
`🔥 *Streak: ${p.streak_days} days* ${streakEmoji(p.streak_days)} ${next?`Next: *${next.days}d* → ${next.r} tries`:'🏆 Max reached!'} Milestones: 3d→+1, 7d→+2, 14d→+3, 30d→+5`,
{parse_mode:‘Markdown’,…kb(p.referral_code)}
);
});
...
`⚔️ *Your Squad* (${m.length}/${SQUAD_SIZE}) ${m.map((x,i)=>`${i+1}. ${maskName(x.username,x.first_name)} — ${x.total_clicks} clicks`).join('\n')} If any member wins → ${SQUAD_PCT}% each 🔗 \`${sLink}``, {parse_mode:'Markdown',...kb(p.referral_code)} ); } const squadCode=genCode(id,'squad'); const {rows:ns}=await pool.query(`INSERT INTO squads(name,creator_id,invite_code) VALUES($1,$2,$3) RETURNING *`, [`${p.first_name||‘Duck’}’s Squad`,id,squadCode] ); await pool.query('UPDATE players SET squad_id=$1 WHERE telegram_id=$2',[ns[0].id,id]); const squadInvite=`https://t.me/xDegenDuck100Bot?start=squad_${squadCode}`; bot.sendMessage(id, `⚔️ *Squad Created!* Need ${SQUAD_SIZE-1} more.
If any member wins 100 SOL → all get ${SQUAD_PCT}% each!
🔗 `${squadInvite}``, {parse_mode:'Markdown',reply_markup:{inline_keyboard:[[ {text:'⚔️ Share Squad Invite',url:`https://t.me/share/url?url=${encodeURIComponent(squadInvite)}&text=${encodeURIComponent(‘⚔️ Join my $DEGEN squad! If any of us wins 100 SOL we all win! 🦆’)}`}
],[{text:‘🦆 Play Now’,web_app:{url:APP_URL}}]]}}
);
});
...
bot.sendMessage(msg.from.id,
`👥 *Invite Stats* ✅ Invites: *${ir[0].cnt}* | 🎯 Tries: *${p.tries_remaining}* 🏅 Top referrer wins *5 SOL on Oct 10!* 🔗 \`${inviteLink}``,
{parse_mode:‘Markdown’,…kb(p.referral_code)}
);
});
...
`🏆 *Leaderboard — Closest to Zero* ${list} 🥇100 SOL 🥈10 SOL 🥉5 SOL 🏅5 SOL (top ref)`,
...
const p=await getOrCreatePlayer(msg.from.id,msg.from.username,msg.from.first_name);
bot.sendMessage(msg.from.id,
`⚡ *Earn tries:* ✅ Join @xDegenDuck → required 👥 Invite friends → +1 each (max ${MAX_BONUS}) 🐦 Follow on X → +2 | 🔁 Retweet → +1 🔥 Streak: 3d+1 7d+2 14d+3 30d+5 📤 Daily share → +1/day 💎 Buy with ⭐ Stars → /buy`,
{parse_mode:‘Markdown’,…kb(p.referral_code)}
);
});
...
bot.sendMessage(msg.chat.id,
`📋 *RULES* 🥇 Last click → 100 SOL 🥈 2nd last → 10 SOL  🥉 3rd last → 5 SOL 🏅 Top referrer Oct 10 → 5 SOL Every click resets the shared 5min timer Squad of ${SQUAD_SIZE} → ${SQUAD_PCT}% each if any member wins Daily streak = bonus tries at milestones Buy tries with ⭐ Stars (/buy) Oct 10 2026 deadline | 1 account per person`,
{parse_mode:‘Markdown’,…baseKb}
);
});
...
const p=await getOrCreatePlayer(msg.from.id,msg.from.username,msg.from.first_name);
bot.sendMessage(msg.from.id,‘🦆 Use the buttons or try /mystats /squad /invite /buy /mycard’,kb(p.referral_code));
});
...
// ── CLICK with CAPTCHA check ──
app.post(’/api/game/click’, async(req,res)=>{
try {
const {telegramId,username,firstName,timerMsLeft}=req.body||{};
const id=Number(telegramId); if(!Number.isFinite(id)) return res.status(400).json({error:‘telegramId required’});
const now=new Date(); if(now.getTime()>=EVENT_END.getTime()) return res.status(403).json({error:‘Event ended’});
...
// Check captcha lock
if (player.captcha_locked_until&&new Date(player.captcha_locked_until)>now) {
  const lockMs=new Date(player.captcha_locked_until)-now;
  return res.status(403).json({error:'captcha_locked',lockMs,lockSec:Math.ceil(lockMs/1000)});
}

// First-time click — send captcha if not verified
if (!player.verified_human) {
  const clickMs=typeof timerMsLeft==='number'?timerMsLeft:null;
  const displayName=firstName||username||player.first_name||player.username||`Player ${id}`;
  pendingClicks.set(id,{clickMs,displayName,ts:Date.now()});
  // Clear pending after 3 min if not answered
  setTimeout(()=>pendingClicks.delete(id),3*60*1000);
  setImmediate(()=>sendCaptcha(id,bot));
  return res.json({success:false,captchaRequired:true,message:'Verification sent to your Telegram!'});
}

// Verified — process click immediately
const clickMs=typeof timerMsLeft==='number'?timerMsLeft:null;
const displayName=firstName||username||player.first_name||player.username||`Player ${id}`;
...
await pool.query('INSERT INTO clicks(telegram_id,timer_ms) VALUES($1,$2)',[id,clickMs]);
const newTimerEnd=new Date(now.getTime()+TIMER_MS);
await getOrInitState();
await pool.query(
  `UPDATE game_state SET timer_ends_at=$1,third_clicker_id=second_clicker_id,third_clicker_name=second_clicker_name,second_clicker_id=last_clicker_id,second_clicker_name=last_clicker_name,last_clicker_id=$2,last_clicker_name=$3,total_clicks=total_clicks+1,updated_at=$4 WHERE id=1`,
...
const {streakDays,bonusEarned,bonus,milestone}=await handleStreak(id);
const updated=(await pool.query('SELECT tries_remaining,total_clicks,best_click_ms,streak_days FROM players WHERE telegram_id=$1',[id])).rows[0];
...
res.json({success:true,triesRemaining:updated.tries_remaining,totalClicks:updated.total_clicks,bestClickMs:updated.best_click_ms,bestClickFmt:fmtMs(updated.best_click_ms),streakDays:updated.streak_days,timerEndsAt:newTimerEnd,remainingMs:TIMER_MS});
```
...
const stop=sig=>{dailyTask.stop();bot.stopPolling().finally(()=>pool.end()).finally(()=>process.exit(0));};
